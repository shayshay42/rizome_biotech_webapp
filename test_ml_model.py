...
Tests cancer classifier with the bundled CatBoost model and fallback simulation
"""
...
def test_classifier_initialization():
    ...
    print(f"  Model type: {'CatBoost' if classifier.model_loaded else 'Simulation'}")
...
def run_all_tests():
    ...
    print("Will use CatBoost if available, otherwise simulation mode")
    ...
    try:
        ...
        if classifier.model_loaded:
            print("\n✅ Using bundled CatBoost model (autogluon_catboost_bag_l1_v1)")
            print("✅ Production-ready predictions")
        else:
            print("\n⚠️  Using SIMULATION mode (rule-based)")
            print("ℹ️  Run scripts/export_autogluon_model.py to regenerate the model artifact")
